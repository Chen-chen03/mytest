...
def introduce_page():
    ...
    st.markdown(
        ...
        ## 使用指南
        - 输入准确完整的的被保险人信息，可以得到更准确的费用预测。
        - 预测结果可以作为保险定价的重要参考，但需审慎决策。
        - 有任何问题欢迎联系我们的技术支持。

        技术支持：emai:: support@exampel.com
        """
    )
...
def predict_page():
    ...
    with st.form('user_inputs'):
        ...
        sex = st.radio("性别", options=["男性", "女性"], index=0)
        bmi = st.number_input("BMI", min_value=0.0, value=0.0, step=0.1)
        children = st.number_input("子女数量", min_value=0, value=0)
        smoke = st.radio("是否吸烟", ["是", "否"], index=1)
        region = st.selectbox("区域", ["东南部", "东北部", "西北部", "西南部"])
        submitted = st.form_submit_button("预测（单击使用）")
    ...
    if submitted:
        ...
        # 使用pickle的load方法从磁盘文件反序列化加载之前保存的随机森林回归模型
        with open('rfr_model.pkl', 'rb') as f:
            rfr_model = pickle.load(f)

        # 将格式化数据转换为DataFrame（匹配模型的特征列）
